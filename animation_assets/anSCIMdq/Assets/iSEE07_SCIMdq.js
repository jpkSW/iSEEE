...
if (reversed == null) { reversed = false; }
	...
	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#000000").s().p("AgQAfQgGgGgBgJIAKgBQABAHADADQAEAEAFAAQAGAAAEgFQAFgFAAgIQAAgHgFgEQgEgEgGAAQgEAAgDACQgEACgCACIgIgBIAHgkIAiAAIAAAJIgbAAIgEATQAHgFAGAAQAJAAAGAHQAHAFAAAKQAAAKgGAHQgHAJgLAAQgJAAgHgFg");
	this.shape_5.setTransform(171.375,589.725);
...
// stage content:
(lib.animateSCIMdq = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
		...
		function init() {
			...
			function createTouchControl() { // button- & slider-stuff
				...
				function createSlider() { // 2 sliders
					...
					sliderfS = new createSliderObj(phBaseX, stopBtn.y, "f_S (p.u.):");
					// http://www.javascripter.net/faq/greekletters.htm
					sliderfS.slider.x += sliderfS.bar.nominalBounds.width / 2;
					sliderfS.txt.x = sliderfS.slider.x;
			...
			function initSlider() { // sliderField + -Torque
				sliderfS.slider.addEventListener("pressmove", function (e) {
					// update GUI, uPn & vecUP.scale => updatePhasors
					let p = stage.globalToLocal(e.stageX, e.stageY);
					let fR = scim.fS - scim.speed;
					sliderfS.slider.x = limitIt(p.x, sliderfS.bar.x - sliderfS.bar.nominalBounds.width / 2,
						sliderfS.bar.x + sliderfS.bar.nominalBounds.width / 2)
					sliderfS.txt.x = sliderfS.slider.x;
					scim.fS = 2*(0.5 + (sliderfS.slider.x - sliderfS.bar.x) / sliderfS.bar.nominalBounds.width);
					scim.fS = step(scim.fS, 0.05);
					if (scim.fS < 0.025) {
						scim.fS = 0.000001;
					}
					if (scim.fS < 1.0) {
					...
					setSlider(sliderSpeed, scim.speed, -1.0, 2.0);
					calcECD();
				});
				...
				function setSlider(sliderX, val, valmin, valmax) {
					if (DBG_SLIDER) console.log("setSlider: val=" + val + ", valmin=" + valmin + ", valmax=" + valmax + 
						", rel=" + ((val - valmin)/(valmax - valmin)));
					if (DBG_SLIDER) console.log("setSlider: sliderX.bar.nominalBounds.width=" + sliderX.bar.nominalBounds.width + 
						", sliderX.bar.x=" + sliderX.bar.x + ", sliderX.slider.x=" + sliderX.slider.x);
					sliderX.slider.x = ((val - valmin)/(valmax - valmin) - 0.5) * sliderX.bar.nominalBounds.width + sliderX.bar.x;
			...
			function calcECD() { // get iS and iR 
				...
				if (scim.fS > 1.0) {
					...
					locusIS.x = vecUS.x + (locusIS.x0 - vecUS.x)/scim.fS;
					// scale locus 
					locusIS.scale = locusIS.scale0 / scim.fS;
				} else { 
					// reset locus position
					locusIS.x = locusIS.x0;
					// reset locus scaling
					locusIS.scale = locusIS.scale0;
				}
				let rRSpd = 9.999;
				...
				if (Math.abs(scim.speed - scim.fS) < 0.01) {
					...
					scim.iR[0] = uS / Math.sqrt(xsig * xsig + rRSpd * rRSpd);
					scim.iR[1] = Math.atan2(xsig, rRSpd) * rad2deg;
					...
					scim.iR[0] = scim.iR[0] / (1.0 - scim.sigma);
					if (rRSpd > 0.0) {
						vecI0.rotation = scim.iR[1];
					} else {
						vecI0.rotation = scim.iR[1] + 180.0;
					}
				}
				// |IS|, phi:
				...
				vecIR.y = vecIS.y - vecISlength * Math.cos(scim.iS[1] / rad2deg) +
									vecIRlength * Math.cos(scim.iR[1] / rad2deg);
				if (DBG_ECD) console.log("scim: sigma=" + scim.sigma + ", x=" + scim.x + 
					", xsig=" + xsig.toFixed(3) + ", rR=" + scim.rR.toFixed(3) + 
					", rRSpd=" + rRSpd.toFixed(3) + ", uS=" + uS.toFixed(3));
				if (DBG_ECD) console.log("scim: iS=[" + scim.iS[0].toFixed(3) + " : " + scim.iS[1].toFixed(0) + 
					"], iR=[" + scim.iR[0].toFixed(3) + " : " + scim.iR[1].toFixed(0) + "]");
			}
